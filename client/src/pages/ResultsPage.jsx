...
export default function ResultsPage() {
  const api = useApi()
  const [results, setResults]         = useState([]) // flat list from API
  const [grouped, setGrouped]         = useState([]) // grouped by event
    ...
    api.get('/api/results')
      .then(data => {
        ...
        const groups = Object.values(map).map(g => ({
          ...g,
          standings: g.standings.sort((a, b) => a.position - b.position),
        }))
        // Sort events by date descending
        groups.sort((a, b) => new Date(b.date) - new Date(a.date))
        setGrouped(groups)
        if (groups.length > 0) setActiveEventId(groups[0].id)
      })
  ...
  const filtered = activeCategory === 'All'
    ? grouped
    : grouped.filter(r => r.category === activeCategory)

  const activeResult = filtered.find(r => r.id === activeEventId) || filtered[0] || null
  ...
  return (
    <DashboardLayout>
      ...
      <div className="pill-row">
        {categories.map(c => (
          <button key={c} onClick={() => { setActiveCategory(c); setActiveEventId(null) }} className={`pill${activeCategory === c ? ' pill--active' : ''}`}>{c}</button>
        ))}
      </div>
        ...
        <div className="results-layout">
            ...
            {filtered.map(r => (
              ...
              >
                ...
                <div className="result-item__medals">
                  {r.standings.slice(0, 3).map((_, i) => (
                    <span key={i}>{podiumConfig[i + 1]?.icon}</span>
                  ))}
                </div>
              </button>
            ))}
          ...
          {activeResult && (
            <div className="result-detail glass-card">
              ...
              {/* Podium visual — top 3 only */}
              {activeResult.standings.length >= 2 && (
              ...
              <div className="standings">
                {activeResult.standings.map(s => {
                  const pc = podiumConfig[s.position] || { icon: `#${s.position}`, color: '#6B6B8A', bg: 'rgba(107,107,138,0.1)', border: 'rgba(107,107,138,0.2)' }
                  return (
                    <div
                      key={s.userId}
                    ...
                    >
                      ...
                      <div className="standing-row__body">
                        ...
                        <p className="standing-row__dept">{s.email}</p>
                      </div>
                      {s.score != null && (
          ...
          {!activeResult && filtered.length === 0 && (
            <div className="glass-card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 200 }}>
              <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>No results published yet.</p>
            </div>
          )}
        </div>
      )}
...
.result-item__cat {
  font-size: 10px; font-weight: 600; color: #7C74FF;
  background: rgba(108,99,255,0.1); padding: 3px 8px;
...
.podium {
  ...
  gap: 12px; height: 140px; margin-bottom: 32px;
}
...
.podium-slot__bar { width: 80px; border-radius: 8px 8px 0 0; }
...
.standing-row__icon { font-size: 24px; width: 32px; text-align: center; flex-shrink: 0; }
